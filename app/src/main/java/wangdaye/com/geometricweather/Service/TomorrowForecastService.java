...
public class TomorrowForecastService extends Service implements HandlerContainer {
    ...
    private void sendSimpleForecast(WeatherInfoToShow info) {
        ...
        // set view
        builder.setSmallIcon(JuheWeather.getMiniWeatherIcon(info.weatherKind[1], isDay));
        RemoteViews view = new RemoteViews(getPackageName(), R.layout.notification_base);
        int[] imageId = JuheWeather.getWeatherIcon(info.weatherKind[1], isDay);
        view.setImageViewResource(R.id.notification_base_image_today, imageId[3]);
        String text1 = "明日" + info.location + " : " + info.weather[1] + " " + info.miniTemp[1] + "/" + info.maxiTemp[1] + "°";
        view.setTextViewText(R.id.notification_base_text_title, text1);
        String text2 = "今日 : " + info.miniTemp[0] + "/" + info.maxiTemp[0] + "°";
        switch (info.weatherKind[1]) {
            case "晴":
                text2 = text2 + " , 明日晴朗。";
                break;
            case "云":
        ...
        notificationManager.notify(TOMORROW_FORECAST_ID, notification);
    }
    ...
    private class MyLocationListener implements BDLocationListener {
        ...
        public void onReceiveLocation(BDLocation location) {
            ...
            if (location.getLocType() == BDLocation.TypeGpsLocation){// GPS定位结果
                ...
                Toast.makeText(TomorrowForecastService.this, "离线定位成功", Toast.LENGTH_SHORT).show();
                locationName = location.getCity();
            } else if (location.getLocType() == BDLocation.TypeServerError) {
                Toast.makeText(TomorrowForecastService.this, "服务端网络定位失败", Toast.LENGTH_SHORT).show();
                sb.append("\ndescribe : ");
                sb.append("服务端网络定位失败，可以反馈IMEI号和大体定位时间到loc-bugs@baidu.com，会有人追查原因");
            } else if (location.getLocType() == BDLocation.TypeNetWorkException) {
                Toast.makeText(TomorrowForecastService.this, "网络连接失败，请检查网络手否连通", Toast.LENGTH_SHORT).show();
                sb.append("\ndescribe : ");
                sb.append("网络不同导致定位失败，请检查网络是否通畅");
            } else if (location.getLocType() == BDLocation.TypeCriteriaException) {
                Toast.makeText(TomorrowForecastService.this, "无法获取信息，或因网络不通或缺失权限所致", Toast.LENGTH_SHORT).show();
                sb.append("\ndescribe : ");
    ...
    public void handleMessage(Message message) {
        switch(message.what)
        {
            case REFRESH_DATA_SUCCEED:
                ...
                SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this);
                String type = sharedPreferences.getString(getString(R.string.key_forecast_type_tomorrow), "simple_forecast");
                Log.d("TomorrowForecastService", type);
                if (location != null) {
