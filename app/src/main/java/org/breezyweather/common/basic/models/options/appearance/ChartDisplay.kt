...
enum class ChartDisplay(
    ...
    TAG_TEMPERATURE("temperature", R.string.temperature, R.drawable.ic_device_thermostat),
    ...
    TAG_OTHER_DETAILS("other_details", R.string.other_details, R.drawable.ic_about),
    ;

    /**
     * Returns true if the hourly list in parameter has enough data for a chart
     * TODO: Take into account DST
     * TODO: Possibly take into account lower than 24 hours available (maybe every 6 hours?)
     */
    fun isValidForChart(hourlyList: List<Hourly>): Boolean {
        return when (this) {
            TAG_TEMPERATURE -> hourlyList.filter { it.temperature?.temperature != null }.size == 24
            TAG_AIR_QUALITY -> hourlyList.filter { it.airQuality?.isValid == true }.size == 24
            TAG_WIND -> hourlyList.filter { it.wind?.isValid == true }.size == 24
            TAG_UV_INDEX -> hourlyList.filter { it.uV?.isValid == true }.size == 24
            TAG_PRECIPITATION -> hourlyList.filter { it.precipitation?.isValid == true }.size == 24
            TAG_HUMIDITY -> hourlyList.filter { it.relativeHumidity != null }.size == 24
            TAG_PRESSURE -> hourlyList.filter { it.pressure != null }.size == 24
            TAG_CLOUD_COVER -> hourlyList.filter { it.cloudCover != null }.size == 24
            TAG_VISIBILITY -> hourlyList.filter { it.visibility != null }.size == 24
