...
object LanguageUtils {
    ...
    init {
        val charS = UTF8S.toCharArray()
        val charT = UTF8T.toCharArray()
        var i = 0
        val n = min(charT.size, charS.size)
        while (i < n) {
            val cT = charT[i]
            val cS = charS[i]
            T2S[cT] = cS
    ...
    fun setLanguage(context: Context, locale: Locale) {
        if (getCurrentLocale(context) != locale) {
    ...
    fun isChinese(text: String?): Boolean {
        ...
        for (c in chars) {
            if (!Pattern.compile("[\u4e00-\u9fa5]").matcher(c.toString()).matches()) {
                return false
            }
        }
        return true
