...
fun SecondarySourcesPreference(
    ...
    val compatibleCurrentSources = sourceManager
        .getConfiguredWeatherSources(SourceFeature.CURRENT, location, currentSource.value)
        .groupBy { if (it is HttpSource) it.continent else null }
        .toSortedMap(continentComparator)
        .mapValues { m ->
            m.value.associate { it.id to it.name }
    ...
    val compatibleAirQualitySources = sourceManager
        .getConfiguredWeatherSources(SourceFeature.AIR_QUALITY, location, currentSource.value)
        .groupBy { if (it is HttpSource) it.continent else null }
        .toSortedMap(continentComparator)
        .mapValues { m ->
            m.value.associate { it.id to it.name }
    ...
    val compatiblePollenSources = sourceManager
        .getConfiguredWeatherSources(SourceFeature.POLLEN, location, currentSource.value)
        .groupBy { if (it is HttpSource) it.continent else null }
        .toSortedMap(continentComparator)
        .mapValues { m ->
            m.value.associate { it.id to it.name }
    ...
    val compatibleMinutelySources = sourceManager
        .getConfiguredWeatherSources(SourceFeature.MINUTELY, location, currentSource.value)
        .groupBy { if (it is HttpSource) it.continent else null }
        .toSortedMap(continentComparator)
        .mapValues { m ->
            m.value.associate { it.id to it.name }
    ...
    val compatibleAlertSources = sourceManager
        .getConfiguredWeatherSources(SourceFeature.ALERT, location, currentSource.value)
        .groupBy { if (it is HttpSource) it.continent else null }
        .toSortedMap(continentComparator)
        .mapValues { m ->
            m.value.associate { it.id to it.name }
    ...
    val compatibleNormalsSources = sourceManager
        .getConfiguredWeatherSources(SourceFeature.NORMALS, location, currentSource.value)
        .groupBy { if (it is HttpSource) it.continent else null }
        .toSortedMap(continentComparator)
        .mapValues { m ->
            m.value.associate { it.id to it.name }
