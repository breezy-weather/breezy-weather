...
class AlertViewHolder(parent: ViewGroup) : AbstractMainCardViewHolder(
    ...
    fun AlertList(
        ...
        Column {
            location.weather!!.currentAlertList.forEachIndexed { index, currentAlert ->
                ListItem(
                    modifier = modifier
                        .clickable(
                            interactionSource = remember { MutableInteractionSource() },
                        ...
                        .padding(
                            top = if (index == 0) dimensionResource(R.dimen.little_margin) else 0.dp,
                            bottom = if (index == location.weather!!.currentAlertList.lastIndex) {
