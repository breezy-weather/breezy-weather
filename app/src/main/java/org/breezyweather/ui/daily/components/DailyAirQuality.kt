...
fun DailyAirQuality(
    ...
    val mappedValues = remember(hourlyList) {
        hourlyList
            ...
            .associate { it.date.time to it.airQuality!!.getIndex()!! }
            .toImmutableMap()
    }
    ...
    LazyColumn(
        ...
        if (mappedValues.size >= ChartDisplay.CHART_MIN_COUNT) {
            item {
                AirQualityChart(location, mappedValues)
            }
        } else {
            item {
                UnavailableChart(
...
private fun AirQualityChart(
    ...
    BreezyLineChart(
        ...
        MarkerLabelFormatterAirQualityDecorator(mappedValues, location, context),
        persistentListOf(
        ...
        topAxisValueFormatter = { _, value, _ ->
            mappedValues.getOrElse(value.toLong()) { null }?.toString() ?: "-"
        }
    )
}
...
private class MarkerLabelFormatterAirQualityDecorator(
    ...
    override fun format(
        ...
        val quantityFormatted = mappedValues.getOrElse(model.x.toLong()) { null }?.toString() ?: "-"
